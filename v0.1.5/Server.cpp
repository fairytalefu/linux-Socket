...
void* sync_process(void *)
{
	...
	strcpy(msg,"begin----");
	printf("connecting to the image process server\n");
	pthread_t  sync_thread_id  = pthread_self();
	printf("the running  sync_process thread id is %d\n",sync_thread_id);
	while(1)
	{	
		while(mClient.m_bConnected)
		{
			mClient.Send(msg, BUFFER_SIZE);
			bzero(msg, sizeof(msg));
			mClient.Recv(msg, BUFFER_SIZE);
			printf("file msg is %s\n",msg);
			bzero(msg, sizeof(msg));
...
void* handle_image_event_process(void *)
{
	...
	pthread_t  handle_thread_id  = pthread_self();
	printf("the running handle_image_event_process thread id is %lu
",handle_thread_id);
...
void* socket_1_process(void*)
{
	...
	pthread_t  socket_1_thread_id  = pthread_self();
	printf("the running socket_1_process thread id is %lu
",socket_1_thread_id);
...
void* socket_2_process(void*)
{
	...
	pthread_t  socket_2_thread_id  = pthread_self();
	printf("the running socket_2_process thread id is %lu
",socket_2_thread_id);
...
int main(int argc, char *argv[])
{
	...
	ret = pthread_create(&syn_id,NULL,sync_process,NULL);
	if(ret != 0)
	{
		perror("socket_2 Thread create failure");
		exit(-1);
	...
	ret = pthread_join( syn_id, &sync_process_result);
	if(ret != 0)
	{
		perror("handle_image_event Thread join failure");
		exit(-1);
